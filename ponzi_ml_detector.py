...
def train_conv_net():
    ...
    optimizer = tf.optim.Adam(conv_net.parameters(), lr=3e-4)
...
def train_ponzi_net():
    ...
    autoencoder = Encoders.StackedAutoEncoder().to(cfg.device)
    criterion = tf.MSELoss()
    optimizer = tf.optim.Adam(ponzi_net.parameters(), lr=3e-4)

    checkpoint = tf.load(cfg.model_path_root, 'conv_net', 'best_model.pth')
...
def test():
    ...
    autoencoder = Encoders.StackedAutoEncoder().to(cfg.device)
    checkpoint = tf.load(cfg.model_path_root + 'ponzi_net' + 'best_model.pth')
